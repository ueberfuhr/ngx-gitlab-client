...
import {DataSet, GitlabService, NoGitlabConnectionProviderError} from './gitlab.service';
...
describe('GitlabService', () => {
  ...
  describe('without Gitlab connection provider', () => {

    const createHttp = createHttpFactory({
      service: GitlabService
    });

    it('should throw an error when Gitlab config provider is not available', () => {
      expect(() => createHttp()).toThrowError(NoGitlabConnectionProviderError);
    })

  });

  describe('with Gitlab config provider', () => {
    ...
    describe('making paginated requests', () => {
      ...
      it('should set options correctly', done => {
        ...
        gitlab.callPaginated('test', {
          ...
          expect(response).toMatchObject({payload: responseBody, index: 0, total: 1} as DataSet<unknown>);
          done();
        });
